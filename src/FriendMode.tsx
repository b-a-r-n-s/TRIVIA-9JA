...
export default function FriendMode({language,onBack}:{language:Language;onBack:()=>void}){
  ...
  const startGame=async(c:Challenge)=>{
    ...
    try{
      ...
      setChallenge(data.challenge);setRemaining(data.challenge.duration_seconds);setPosition(1);setFinished(false);setView('game')
  ...
  const join=async()=>{
    ...
    setBusy(true);setError('')
    try{const data=await call<{challenge:Challenge}>('join_friend_challenge',{invite_code:clean});setChallenge(data.challenge);setCode(clean);await startGame(data.challenge)}
  ...
  useEffect(()=>{
    if(view!=='waiting'||!challenge||challenge.opponent_id)return
    const poll=async()=>{try{const {data}=await supabase.from('friend_challenges').select('id,creator_id,opponent_id,language,duration_seconds,status,started_at,expires_at').eq('id',challenge.id).maybeSingle();if(data?.status==='ready'&&data.opponent_id)void startGame(data as Challenge)}catch{}}
    const t=window.setInterval(poll,1000);void poll();return()=>window.clearInterval(t)
  },[view,challenge?.id,challenge?.opponent_id])
  ...
  if(view==='game'&&challenge)return <main className="app-shell inner-screen"><header className="inner-header"><button onClick={onBack}>←</button><div><p className="eyebrow">FRIEND CHALLENGE</p><h1>{remaining}s left</h1></div><span className="coin-pill">{score} — {otherScore}</span></header><div className="game-progress"><span style={{width:`${Math.max(0,(remaining/challenge.duration_seconds)*100)}%`}}/></div>{error&&<p className="auth-message">{error}</p>}{question?<section className="question-stage"><div className="question-meta"><span>QUESTION {position}</span><span>90 SEC</span></div><div className="question-card"><span className="question-type">{question.question_type==='riddle'?'BRAIN TEASER':question.question_type==='scramble'?'LETTER SCRAMBLE':'FRIEND DUEL'}</span><h1>{question.prompt}</h1></div><div className="answers">{question.options.map((o,i)=><button disabled={Boolean(selected)||busy} className={`answer ${selected===o?(correct?'correct':'wrong'):''}`} key={o} onClick={()=>answer(o)}><span>{String.fromCharCode(65+i)}</span>{o}</button>)}</div></section>:<p className="auth-message">Finishing challenge…</p>}</main>

  if(view==='result')return <main className="app-shell inner-screen"><header className="inner-header"><button onClick={reset}>←</button><div><p className="eyebrow">FRIEND CHALLENGE</p><h1>Challenge complete.</h1></div></header><section className="question-card" style={{textAlign:'center'}}><p className="section-label">FINAL SCORE</p><h1 style={{fontSize:58,margin:'12px 0'}}>{score} — {otherScore}</h1><p>{winner===null?'It’s a draw.':'Winner decided.'}</p><button className="primary-action" onClick={reset}>PLAY AGAIN <span className="arrow">→</span></button></section></main>
