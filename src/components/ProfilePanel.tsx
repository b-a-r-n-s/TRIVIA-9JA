...
export default function ProfilePanel({ onClose }: { onClose: () => void }) {
  ...
  const submitAuth = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (result.error) setAuthError(result.error.message)
      ...
      setAuthMode(null); setAuthEmail(''); setAuthPassword(''); setAnonymous(false)
      await loadProfile()
    }
    setAuthBusy(false)
  }
  ...
  return <div className="profile-overlay">
    <section className="profile-panel">
      ...
      <div className="profile-identity">
        ...
        <div><strong>@{username || 'player'}</strong><span>{anonymous ? 'Guest player' : 'Naija knowledge seeker'}</span></div>
      </div>

      {anonymous && <div className="account-gate">
        <p className="eyebrow">PLAYER ACCESS</p>
        <h3>Save your progress.</h3>
        <p>Guest play is currently unavailable on this build. Sign in or create a player account to continue.</p>
